...
def check_coll(nub_point, scene_tree):
    ...
    #   넙죽이와 기존 PC간의 충돌 확인인 
    #
    #   이거 나중에 scene_tree방식 말고 bounding box로 바꿔야 될지도? 일단 짠거 아쉬우니 이대로 고고헛
    #
    ...
    d, _  = scene_tree.query(nub_point, distance_upper_bound=0.05)
    coll_count = np.sum(d != np.inf)
    return coll_count <= 0.05 * len(nub_point)
...
def find_place(nub_xyz,scene_xyz,scene_normal,scene_tree,nub_center,scene_label):
    ...
    #output:pc - 합친 point cloud
    ##################################################
    bg_mask = scene_label == 0
    bg_xyz = scene_xyz[bg_mask]
    bg_normal = scene_normal[bg_mask]
    
    
    candidate_point = candy(bg_xyz,bg_normal)
